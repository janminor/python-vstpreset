...
class VST3Preset():
    def __init__(self, class_id: str, header_id: str = "VST3", version: int = 1, chunks: dict = None, chunklist_id: str = "List"):
    ...
    def vstpreset(self) -> bytes:
        ...
        out += pack(HEADER_FMT, self.header_id.encode(), self.version, self.class_id.encode(), HEADER_SIZE + len(chunks))
        out += chunks
...
def parse_vstpreset_file(filename: str) -> VST3Preset:
    ...
    return parse_vstpreset(presetdata)

def parse_vstpreset(presetdata: bytes) -> VST3Preset:
    ...
    (header_id, version, class_id, chunklist_offset) = unpack_from(HEADER_FMT, presetdata, offset=0)
    preset = VST3Preset(class_id.decode(), header_id.decode(), version)

    (cl_list_id, cl_entry_count) = unpack_from(CHUNKLIST_HEADER, presetdata, offset=chunklist_offset)
    preset.chunklist_id = cl_list_id.decode()

    start_chunk = chunklist_offset + CL_HEADER_SIZE
    for _ in range(1, cl_entry_count + 1):
        (chunk_id, chunk_offset, chunk_size) = unpack_from(CHUNKLIST_ENTRY, presetdata, offset=start_chunk)
